...
gulp.task('release-clean', function () {
    return del(
      [process.env.RELEASE + process.env.CURRENT_PLATFORM, zipName( process.env.RELEASE + process.env.CURRENT_PLATFORM)], 
      {force: true}
    );
  });
...
gulp.task('release-platform', function(cb) {
    ...
    paths.forEach(path => {
        ...
        fileList.push('../Builds/Runner/' + path + "/**/*");

    });

    var dest = process.env.RELEASE + process.env.CURRENT_PLATFORM;
...
gulp.task('release-runner', function(){
    ...
    var src =  process.env.STAGING = "../Builds/Runner";
...
gulp.task('release-zip', function(){
  ...
  .pipe(gulp.dest(process.env.RELEASE));

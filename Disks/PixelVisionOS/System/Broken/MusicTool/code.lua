...
function Init()
  ...
  if(success == true) then
    ...
    firstSongEndButton.cachedMetaSpriteIds.disabled = FindMetaSpriteId("songstart")
    -- songEndButtons =
    ...
    pickerData = editorUI:CreatePicker({x = 0, y = 112, w = 256, h = (8 * totalTracks)}, 8, 8, totalBeats * totalTracks, "notepicker", 0, "Select a track and beat to preview a note value.")
    pickerData.overShift = 0
    ...
    pixelVisionOS:LoadError(toolName)
...
function DrawNote(track, beat, selected, disabled, muted)
  ...
  if(spriteId > -1)then
    DrawMetaSprite(spriteId, beat, track + pickerData.tiles.r, false, false, DrawMode.Tile)
  end
...
function GenerateSong()
  ...
  -- pixelVisionOS:ShowMessageModal("Generate Music", "This will clear the current tracker, generate a new random song, and reset some of your sound effects to default musical instruments. Are you sure you want to do this?", 160, true,
  --   function()
  --     if(pixelVisionOS.messageModal.selectionValue == true) then

  --       gameEditor:GenerateSong()
  --       LoadLoop(_G["currentPatternID"])
  --       Invalidate()

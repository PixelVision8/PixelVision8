...
function ConfigureModal:Open()
  ...
  DrawSprites(settingspanel.spriteIDs, self.rect.x + 8, self.rect.y + 16, settingspanel.width, false, false, DrawMode.TilemapCache)
...
function OnExportSong(songID, showWarning)
  ...
  if(showWarning == true) then
    ...
    pixelVisionOS:ShowMessageModal("Export Song", "Do you want to export this song to '"..workspacePath.Path.."'?", 200, true,
      function()
        if(pixelVisionOS.messageModal.selectionValue == true) then
...
function OnResetConfig()
  ...
  pixelVisionOS:ShowMessageModal("Reset Music Generator", "Do you want to restore the music generator to its default values?", 160, true,
    function()
      if(pixelVisionOS.messageModal.selectionValue == true) then
        -- Save changes

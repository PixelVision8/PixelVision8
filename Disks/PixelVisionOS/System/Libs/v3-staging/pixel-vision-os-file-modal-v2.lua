...
function NewFileModal:SetText(title, inputText, message, editable, okLabel, cancelLabel)
...
function NewFileModal:Open()
    ...
    local backBtnData = self.editorUI:CreateTextButton({x = self.rect.x, y = self.rect.y + 48}, self.okLabel, "Accept the changes.", PaletteOffset(1))
    ...
    local cancelBtnData = self.editorUI:CreateTextButton({x = self.rect.x, y = self.rect.y + 48}, self.cancelLabel, "Cancel renaming.", PaletteOffset(1))
    ...
    local nextX = self.rect.x + self.rect.w - 8

    -- Lay out buttons
    for i = 1, #self.buttons do
        local tmpButton = self.buttons[i]
        nextX = math.floor((nextX - tmpButton.rect.w - 8) / 8) * 8

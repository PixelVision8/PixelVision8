...
function PixelVisionOS:OnItemPickerHorizontalScroll(data, value)
  ...
  if(data.hSlider ~= nil) then
    ...
    local scaleOffset = (data.maxScale / ((data.realWidth - data.rect.width) / 8)) * 100
...
function PixelVisionOS:OnItemPickerVerticalScroll(data, value)
  ...
  if(data.vSlider ~= nil) then
    ...
    local scaleOffset = (data.maxScale / ((data.realHeight - data.rect.height) / 8)) * 100
...
function PixelVisionOS:ChangeItemPickerScale(data, scale)
  ...
  data.picker.columns = math.floor(data.picker.rect.width / tmpItemSize)
  data.picker.rows = math.floor(data.picker.rect.height / tmpItemSize)

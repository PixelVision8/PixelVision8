...
function EditorUI:CreateStringStepper(rect, inputWidth, value, options, spriteNamePrefix, toolTip)
  ...
  data.backButton = self:CreatePaletteButton({x = rect.x, y = rect.y}, spriteNamePrefix .. "stepperback", "Decrease the value.", PaletteOffset( 1))
  ...
  data.nextButton = self:CreatePaletteButton({x = data.inputField.rect.x + data.inputField.rect.width, y = rect.y}, spriteNamePrefix .. "steppernext", "Increase the value.", PaletteOffset( 1))

...
function EditorUI:CreateTextEditor(rect, text, toolTip, font, colorOffset)
  ...
  -- Be default, text colors should be right after the main 16 system colors unless overwritten when creating the component
  local colorShift = colorOffset or 16
  ...
  data.highlighterTheme = {
    ...
    text = colorShift + 2,
    disabled = colorShift + 4,
    -- Code highlight
    string = colorShift,
    comment = colorShift + 4,
    number = colorShift + 6,
    keyword = colorShift + 8,
    api = colorShift + 10,
  ...
  data.viewPort = NewRect(data.rect.x, data.rect.y, data.rect.width, data.rect.height)
  ...
  data.keymap = {
    ...
    ["pageup"] = function(targetData)
      targetData.vy = targetData.vy - targetData.tiles.height
      targetData.cy = targetData.cy - targetData.tiles.height
    ...
    ["pagedown"] = function(targetData)
      ...
      local bottom = #targetData.buffer - targetData.tiles.height + 1
...
function EditorUI:TextEditorCheckPosition(data)
  ...
  if data.cy > data.tiles.height + data.vy - 1 then --Passed the screen to the bottom
    data.vy = data.cy - (data.tiles.height - 1); flag = true
  ...
  if data.cx > data.tiles.width + (data.vx - 1) then --Passed the screen to the right
    data.vx = data.cx - (data.tiles.width - 1); flag = true
...
function EditorUI:TextEditorDrawBlink(data)
  ...
  if data.sxs or data.cx > (data.vx + (data.tiles.width - 1)) then return end

  if data.cy - data.vy < 0 or data.cy - data.vy > data.tiles.height then return end
  if data.bflag then
...
function EditorUI:TextEditorDrawBuffer(data)
  ...
  local vbuffer = lume.slice(data.buffer, data.vy, data.vy + data.tiles.height - 1) --Visible buffer
...
function EditorUI:TextEditorDrawLine(data)
  ...
  -- Reset validation
  if data.cy - data.vy < 0 or data.cy - data.vy > data.tiles.height - 1 then return end
...
function EditorUI:TextEditorMouseMoved(data, cx, cy)--, dx, dy, it)
  ...
  if(data.cx ~= cx2 or data.cy ~= cy2) then
    ...
    if not data.sxs then --Start the selection
      ...
      if cy > data.tiles.height - 2 then
        data.sflag.y = 1
      elseif cy < 1 then
        data.sflag.y = -1
      else
    ...
    if(self.editingField ~= nil and data.name == self.editingField.name and data.editing == true) then
      ...
      elseif data.sflag.x ~= 0 or data.sflag.y ~= 0 then -- if selecting with the mouse and scrolling up/down
        ...
        if data.stimer > data.stime then
          ...
          data.vx = Clamp(data.vx + data.sflag.x, 1, data.maxLineWidth - data.tiles.width)

          data.vy = Clamp(data.vy + data.sflag.y, 1, #data.buffer - data.tiles.height)
...
function EditorUI:ResizeTexdtEditor(data, width, height, x, y)
  ...
  if(data.rect.x == x and data.rect.y == y and data.rect.width == width and data.rect.height == height) then
    return
  end
  ...
  data.tiles.x = math.floor(data.rect.x / data.charSize.x)
  data.tiles.y = math.floor(data.rect.y / data.charSize.y)
  data.tiles.width = math.ceil(data.rect.width / data.charSize.x)
  data.tiles.height = math.ceil(data.rect.height / data.charSize.y)
  data.viewPort = NewRect(data.rect.x, data.rect.y, data.rect.width, data.rect.height)
  data.bgMaskDrawArguments[1] = data.rect.x
  data.bgMaskDrawArguments[2] = data.rect.y
  data.bgMaskDrawArguments[3] = data.rect.width
  data.bgMaskDrawArguments[4] = data.rect.height
  data.lineMaskDrawArguments[1] = data.rect.x
  data.lineMaskDrawArguments[2] = data.rect.y
  data.lineMaskDrawArguments[3] = data.rect.width
  data.lineMaskDrawArguments[4] = data.rect.height

...
namespace MonoGameRunner.Data
{
    public class DisplayTarget : IDisplayTarget
    {
        ...
        public float brightness
        {
            get
            {
                return shaderEffect?.Parameters["brightboost"]?.GetValueSingle() ?? 0;
            }
            set
            {
                shaderEffect?.Parameters["brightboost"]?.SetValue(MathUtil.Clamp(value, .5f, 1.5f));
            }
        }
        ...
        public float sharpness
        {
            get
            {
                return shaderEffect?.Parameters["hardPix"]?.GetValueSingle() ?? 0;
            }
            set
            {
                shaderEffect?.Parameters["hardPix"]?.SetValue(value);
            }
        }
...
//                Effect tmpEffect;
                ...
                using (var reader = new BinaryReader (File.Open (value, FileMode.Open))) {
                    crtShader = new Effect (graphicManager.GraphicsDevice, reader.ReadBytes ((int)reader.BaseStream.Length));
                }
...
//        private readonly GraphicsDeviceManager _graphicsDeviceManager;
        ...
        public DisplayTarget(GraphicsDeviceManager graphicManager, int width, int height, bool fullscreen = false)
        {
            ...
            _monitorWidth = MathUtil.Clamp(width,64, 640);
            _monitorHeight = MathUtil.Clamp(height, 64, 480);
//            monitorScale = scale;
...
//        private GameWindow window;
        ...
        public void ResetResolution(IEngine activeEngine, int monitorScale, bool fullscreen, bool matchResolution, bool stretch)
        {
            ...
            var tmpMonitorScale = fullscreen ? 1 : MathUtil.Clamp(monitorScale, 1, 4);
...
//            Console.WriteLine("Reset Res Fullscreen " + fullscreen + " "+displayWidth+"x"+displayHeight);
            ...
            shaderEffect?.Parameters["outputSize"].SetValue(new Vector2(graphicManager.PreferredBackBufferWidth, graphicManager.PreferredBackBufferHeight));

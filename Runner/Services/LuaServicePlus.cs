...
namespace PixelVision8.Runner.Services
{
    public class LuaServicePlus : LuaService
    {
        ...
        {
            get { return desktopRunner.workspaceService; }
        }
...
//            ".p8"
        ...
        public override void ConfigureScript(Script luaScript)
        {
            ...
            luaScript.Globals["ImportColorsFromGameEditor"] = (ImportColorsFromGameEditorDelegator) ImportColorsFromGameEditor;
...
//            luaScript.Globals["IsExporting"] = new Func<bool>(IsExporting);
            ...
            luaScript.Globals["SessionID"] = new Func<string>(ReadSessionID);
            
            
            
//            luaScript.Globals["ReadFPS"] = new Func<int>(runner.activeEngine.gameChip.ReadFPS);
            luaScript.Globals["BootDone"] = new Action(desktopRunner.BootDone);
            luaScript.Globals["ClearLog"] = new Action(workspace.ClearLog);
            
            luaScript.Globals["ReadPreloaderPercent"] = new Func<int>(desktopRunner.ReadPreLoaderPercent);
//            luaScript.Globals["ReadPreloaderMessage"] = new Func<string>(runner.ReadPreLoaderMessage);
            luaScript.Globals["ShutdownSystem"] = new Action(desktopRunner.ShutdownSystem);
            ...
            luaScript.Globals["StartNextPreload"] = new Action(desktopRunner.StartNextPreload);
            luaScript.Globals["PreloaderComplete"] = new Action(desktopRunner.PreLoaderComplete);
...
//            luaScript.Globals["ValidateGameInDir"] = (ValidateGameInDirDelegator) ValidateGameInDir;
            ...
            luaScript.Globals["DocumentPath"] = new Func<string>(ReadDocumentPath);
            luaScript.Globals["TmpPath"] = new Func<string>(ReadTmpPath);
...
//            luaScript.Globals["Sharpness"] = (SharpnessDelegator)Sharpness;
        ...
        {
            return desktopRunner.sessionID;
        }
        
        public string SystemVersion()
        {
            return desktopRunner.systemVersion;
        }

        public string SystemName()
        {
            return desktopRunner.systemName; 
        }
        
        public string ReadDocumentPath()
        {
            return workspace.documentsPath;
        }
        
        public string ReadTmpPath()
        {
            return workspace.tmpPath;
        }
...
//                }
                ...
                if (srcPath != destPath)
                {
                    fileSystem.Copy(srcPath, fileSystem, destPath);
                }
        ...
        public bool DeleteFile(string src, bool autoDelete = false)
        {
            ...
            try
            {
                ...
                if (autoDelete == false)
                {
                    ...
                    if (!workspace.fileSystem.Exists(destPath))
                    {
                        workspace.fileSystem.CreateDirectory(destPath);
                    }
...
// TODO need to rewire mapping action keys since this should be more generic
        ...
        {
            var success = desktopRunner.Load(path, RunnerGame.RunnerMode.Playing, metaData);
        ...
        /// </summary>
        /// <param name="metaData">An optional argument to supply additional metadata back to the tool that is loaded when quitting.</param>
        /// <param name="tool">An optional argument that allows you to supply a string name for a built-in tool to load instead of the default workspace explorer.</param>
...
//        }
        ...
        public int ImportColorsFromGameEditor(int? resetIndex = null)
        {
            ...
            if (resetIndex.HasValue && resetIndex.Value > 0)
            {
                colorChip.total = resetIndex.Value;
            }
...
//            Console.WriteLine("Searching for editors");
            ...
            {
                paths.Add(FileSystemPath.Parse(disk.Value).AppendDirectory("Tools"));
            }
...
//                    runner.DisplayWarning("Couldn't find editor path " + path);
        ...
        private List<string> supportedExportFiles = new List<string>()
        {
            "sprites.png",
            "tilemap.png",
            "tilemap.json",
                            ...
                            foreach (var file in files)
                            {
                                ...
                                try
                                {
                                    ...
                                    if (file.IsFile)
                                    {
                                        ...
                                        if (supportedExportFiles.IndexOf(fileName) > -1 || fileName.EndsWith(".lua") || fileName.EndsWith(".font.png"))
                                        {
...
//            "system-colors.png",
        ...
        ///     file types will be returned.</param>
        /// <returns></returns>

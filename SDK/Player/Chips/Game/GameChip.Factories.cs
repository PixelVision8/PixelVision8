...
namespace PixelVision8.Player
{
    ...
    public partial class GameChip
    {
        ...
        public SpriteCollection NewMetaSprite(string name, int[] spriteIDs, int width = 1, int colorOffset = 0)
        {

            var id = -1;

            for (int i = 0; i < TotalMetaSprites; i++)
            {
                if(MetaSprite(i).Name == "EmptyMetaSprite")
                {
                    id = i;
                    break;
                }
